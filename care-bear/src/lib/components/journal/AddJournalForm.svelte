...
<form on:submit|preventDefault={handleSubmit} class={`p-4 bg-fuchsia-100 shadow rounded-lg m-4 ${darkModeClass}`}>
    <h2 class="text-2xl font-semibold mb-4 ml-0">Create a new journal</h2>
    <div class="mb-4 ml-0">
        <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 ml-0">Title</label>
        <input
            type="text"
            id="title"
            bind:value={title}
    ...
    <div class="mb-4 ml-0">
        <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 ml-0">Description</label>
        <textarea
            id="description"
